...
(function () {
    const init = () => {
        ...
        codeBlocks.forEach(codeBlock => {
            ...
            if (!pre || pre.querySelector('.antigravity-copy-btn')) return;
        ...
        const menuConfig = [
            { label: 'Copy (コピー)', command: 'antigravity.copyParams' },
            { separator: true },
            { label: 'スライドショー開始', command: 'antigravity.startSlideshow' },
            { label: 'スライド一覧 (Overview)', internal: true, action: 'showOverview' },
            { separator: true },
            { label: 'HTML出力 (Folder)', command: 'antigravity.exportToHtmlFolder' },
            { label: 'HTML出力 (BASE64)', command: 'antigravity.exportToHtmlBase64' },
                ...
                submenu: [
                    ...
                    },
                    {
                        ...
                        submenu: [
                            ...
                            { label: 'Report', theme: 'Report' },
                            { label: 'Minutes', theme: 'Minutes' },
                            { label: 'Proposal', theme: 'Proposal' }
                        ]
                    },
                    {
        ...
        function buildMenu(container, items) {
            items.forEach(item => {
                ...
                if (item.submenu) {
                    ...
                    div.className = 'antigravity-menu-item has-submenu';
                ...
                a.className = 'antigravity-menu-item';
                ...
                const isCopy = (item.command === 'antigravity.copyParams');
                ...
                if (isCopy) {
                    ...
                    if (item.command) {
                        ...
                        a.href = `command:antigravity.setTheme?${encodeURIComponent(JSON.stringify([item.theme]))}`;
                    }
        ...
        function parseSlides() {
            log('Parsing slides...');
            const content = document.getElementById('antigravity-content');
            if (!content) return;

            // Clear old slides
            const oldSlides = document.querySelectorAll('.antigravity-slide');
            ...
            function flushSlide(nextStartLine = -1) {
                ...
                slideDiv.className = 'antigravity-slide';
                const inner = document.createElement('div');
                inner.className = 'antigravity-slide-inner';

                // Process Nodes: Group consecutive .antigravity-column into a wrapper
                const processedNodes = [];
                let columnGroup = null;

                currentSlideNodes.forEach(node => {
                    const isColumn = node.nodeType === 1 && node.classList.contains('antigravity-column');

                    if (isColumn) {
                        if (!columnGroup) {
                            columnGroup = document.createElement('div');
                            columnGroup.className = 'antigravity-columns'; // Re-use generic columns class
                ...
                processedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        if (node.classList.contains('antigravity-container')) {
                            ...
                            // we must inspect its children to group .antigravity-column elements.
                            ...
                            children.forEach(child => {
                                ...
                                const isCol = child.nodeType === 1 && child.classList.contains('antigravity-column');
                                ...
                                if (isCol) {
                                    if (!innerColumnGroup) {
                                        ...
                                        innerColumnGroup.className = 'antigravity-columns';
                        ...
                        if (node.classList.contains('note')) {
                            node.classList.add('antigravity-note');
                        }
                    }
                });
        ...
        function setThemeClass(theme) {
            ...
            body.classList.forEach(cls => {
                if (cls.startsWith('antigravity-theme-')) body.classList.remove(cls);
            });
            body.classList.add(`antigravity-theme-${theme.toLowerCase()}`);
        ...
        window.addEventListener('click', (e) => {
            if (!e.target.closest('.antigravity-menu')) {
                menu.classList.remove('visible');
            }
        ...
        function renderCharts() {
            ...
            graphContainers.forEach((container, idx) => {
                try {
                    ...
                    const mainColor = style.getPropertyValue('--ag-color-main').trim() || '#1A365D';
                    const accentColor = style.getPropertyValue('--ag-color-accent').trim() || '#F6AD55';
        ...
        // Initial setup
        const marker = document.getElementById('antigravity-theme-marker');
        ...
        const stateMarker = document.getElementById('antigravity-state-marker');
